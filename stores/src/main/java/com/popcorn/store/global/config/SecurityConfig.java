...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        HttpSecurity httpSecurity = http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/api/stores/v1/goods/*/price", "/api/stores/v1/sessions/*/price").permitAll() // 공개 가격 조회 API
    ...
    @Value("${store.cors.allowed-origin-patterns:http://localhost:8080,http://127.0.0.1:8080}")
    private String allowedOriginPatterns;

    @Bean
    public CorsConfigurationSource corsConfigurationSource() {
        CorsConfiguration configuration = new CorsConfiguration();
        var allowedOrigins = Arrays.stream(allowedOriginPatterns.split(","))
                .map(String::trim)
