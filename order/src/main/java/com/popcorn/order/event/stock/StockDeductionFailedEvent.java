 ...
 *
 * [이벤트 발행 시점]
 * - Store 서비스에서 재고 차감이 실패했을 때
 * - Redis Stream을 통해 Order 서비스로 전송
 *
 * [이벤트 수신자]
 * - Order 모듈: 재고 차감 실패 처리 및 주문 취소 또는 보상 트랜잭션
 *
 * [Payload 정보]
 * - eventId: 이벤트 고유 ID
 * - orderId: 주문 ID
...
public class StockDeductionFailedEvent extends BaseOrderEvent {
     ...
     * 팩토리 메서드
     */
    public static StockDeductionFailedEvent create(UUID orderId, String reason, Boolean retryable) {
        return create(orderId, reason, retryable, null);
    }

    public static StockDeductionFailedEvent create(UUID orderId, String reason, Boolean retryable, Long userId) {
        return new StockDeductionFailedEvent(orderId, reason, retryable, LocalDateTime.now(), userId);
    }

    /**
     * 재시도 가능한 실패 이벤트 생성
     */
    public static StockDeductionFailedEvent createRetryable(UUID orderId, String reason) {
     ...
     * 재시도 불가능한 실패 이벤트 생성
     */
    public static StockDeductionFailedEvent createNonRetryable(UUID orderId, String reason) {
