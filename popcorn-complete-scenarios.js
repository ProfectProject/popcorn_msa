...
/**
 * =========================================
 * Popcorn 완전한 부하 시나리오 - 100% 성공률 목표
 * =========================================
 * 3단계: 250→800→1800 RPS
 * 3시나리오: hot, dist, fault
 * 로그인: 완전 자동화
 */
...
const POPUP_HOT_ID = __ENV.POPUP_HOT_ID || "07c79042-f179-452e-9318-0d3abb403c44"; // 야식 배달 기술 전시
...
function safeLogin(maxRetries = 5) {
  for (let i = 0; i < maxRetries; i++) {
    ...
    const backoff = Math.min(Math.pow(2, i + 1), 32); // 2,4,8,16,32초
    console.log(`🔄 Login retry ${i + 1}/${maxRetries} in ${backoff}s...`);
    sleep(backoff);
  }
  return false;
}
...
/**
 * =========================================
 * 실제 Popcorn API 호출 (문서 기준 정확한 경로)
 * =========================================
 */
...
export const options = {
  ...
  thresholds: {
    ...
    t_popup_list: ["p(95)<800"],           // 목록 p95 < 800ms
    t_popup_detail: ["p(95)<1000"],        // 상세 p95 < 1초
    t_order_create: ["p(95)<1200"],        // 주문 p95 < 1.2초
    t_payment_req: ["p(95)<1500"],         // 결제 p95 < 1.5초
  },
};
...
export function setup() {
  if (AUTO_LOGIN) {
    console.log(`🔐 Complete auto login with ${LOGIN_ROLE} account...`);
    ...
    console.error("❌ Setup login failed - continuing with public APIs only");
  }
  return null;
}
...
export function stageRush(data) {
  ...
  group("stage2_rush", () => {
    // 토큰 재검증 (10% 확률로)
    if (!globalAccessToken && Math.random() < 0.10) {
      safeLogin(2);
    }
...
export function stageSpike(data) {
  ...
  group("stage3_spike", () => {
    // 토큰 재검증 (15% 확률로 - 스파이크에서 더 자주)
    if (!globalAccessToken && Math.random() < 0.15) {
      safeLogin(3);
    }
